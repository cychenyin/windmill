...
class DummyDispatcher(BaseDispatcher):
    def __init__(self, **args):
        super(DummyDispatcher, self).__init__()
        self.args = args
...
class TestBaseScheduler(object):
    ...
    def test_start(self, scheduler, create_job):
        scheduler._dispatchers = {'exec1': MagicMock(BaseDispatcher), 'exec2': MagicMock(BaseDispatcher)}
        ...
        scheduler._dispatchers['exec1'].start.assert_called_once_with(scheduler, 'exec1')
        scheduler._dispatchers['exec2'].start.assert_called_once_with(scheduler, 'exec2')
        ...
        assert len(scheduler._dispatchers) == 3
        assert len(scheduler._jobstores) == 3
        assert 'default' in scheduler._dispatchers
    ...
    def test_shutdown(self, scheduler, wait):
        ...
        for dispatcher in scheduler._dispatchers.values():
            dispatcher.shutdown.assert_called_once_with(wait)
    ...
    @pytest.mark.parametrize('stopped', [True, False], ids=['stopped=True', 'stopped=False'])
    def test_add_dispatcher(self, scheduler, stopped):
        scheduler._stopped = stopped
        dispatcher = DebugDispatcher()
        dispatcher.start = MagicMock()
        scheduler.add_dispatcher(dispatcher)

        assert scheduler._dispatchers == {'default': dispatcher}
        if not stopped:
            dispatcher.start.assert_called_once_with(scheduler, 'default')
        else:
            assert dispatcher.start.call_count == 0

    def test_add_dispatcher_already_exists(self, scheduler):
        dispatcher = DebugDispatcher()
        scheduler.add_dispatcher(dispatcher)
        exc = pytest.raises(ValueError, scheduler.add_dispatcher, dispatcher)
        assert str(exc.value) == 'This scheduler already has an dispatcher by the alias of "default"'

    def test_remove_dispatcher(self, scheduler):
        scheduler.add_dispatcher(DebugDispatcher(), 'foo')
        scheduler._dispatch_event = MagicMock()
        scheduler.remove_dispatcher('foo')

        assert scheduler._dispatchers == {}
        assert scheduler._dispatch_event.call_count == 1
        event = scheduler._dispatch_event.call_args[0][0]
        assert event.code == EVENT_EXECUTOR_REMOVED
        assert event.alias == 'foo'

    def test_remove_dispatcher_nonexistent(self, scheduler):
        pytest.raises(KeyError, scheduler.remove_dispatcher, 'foo')
...
"""
    ...
    @pytest.mark.parametrize('config', [
        ...
        },
        {
            ...
            'dispatchers': {
                'default': DummyDispatcher(arg1='3', arg2='a'),
                'alter': DummyDispatcher(arg='true')
            },
            'jobstores': {
    ...
    def test_configure_private(self, scheduler, config):
        ...
        assert set(six.iterkeys(scheduler._dispatchers)) == set(['default', 'alter'])
        assert scheduler._dispatchers['default'].args == {'arg1': '3', 'arg2': 'a'}
        assert scheduler._dispatchers['alter'].args == {'arg': 'true'}
    ...
    def test_configure_private_invalid_dispatcher(self, scheduler):
        exc = pytest.raises(TypeError, scheduler._configure, {'dispatchers': {'default': 6}})
        assert str(exc.value) == "Expected dispatcher instance or dict for dispatchers['default'], got int instead"
    ...
    def test_create_default_dispatcher(self, scheduler):
        dispatcher = scheduler._create_default_dispatcher()
    ...
    def test_lookup_dispatcher(self, scheduler):
        dispatcher = object()
        scheduler._dispatchers = {'dispatcher': dispatcher}
        assert scheduler._lookup_dispatcher('dispatcher') is dispatcher

    def test_lookup_dispatcher_nonexistent(self, scheduler):
        pytest.raises(KeyError, scheduler._lookup_dispatcher, 'dispatcher')
...
class TestProcessJobs(object):
    @pytest.fixture
    def job(self):
        job = MagicMock(Job, id=999, dispatcher='default')
    ...
    @pytest.fixture
    def dispatcher(self, scheduler):
        dispatcher = MagicMock(BaseDispatcher)
        scheduler._dispatchers['default'] = dispatcher
        return dispatcher

    def test_nonexistent_dispatcher(self, scheduler, jobstore, logstream):
        """Tests that an error is logged and the job is removed from its job store if its dispatcher is not found."""

        assert scheduler._process_jobs() is None
        jobstore.remove_job.assert_called_once_with(999)
        assert logstream.getvalue() == \
            'Dispatcher lookup ("default") failed for job "job 999" -- removing it from the job store\n'

    def test_max_instances_reached(self, scheduler, job, jobstore, dispatcher, logstream):
        """Tests that a warning is logged when the maximum instances of a job is reached."""

        dispatcher.submit_job = MagicMock(side_effect=MaxInstancesReachedError(job))

        assert scheduler._process_jobs() is None
        assert logstream.getvalue() == \
            'Execution of job "job 999" skipped: maximum number of running instances reached (1)\n'

    def test_dispatcher_error(self, scheduler, job, jobstore, dispatcher, logstream):
        """Tests that if any exception is raised in dispatcher.submit(), it is logged."""

        dispatcher.submit_job = MagicMock(side_effect=Exception('test message'))

        assert scheduler._process_jobs() is None
        assert 'test message' in logstream.getvalue()
        assert 'Error submitting job "job 999" to dispatcher "default"' in logstream.getvalue()

    def test_job_update(self, scheduler, job, jobstore, dispatcher, logstream, freeze_time):
    ...
    def test_wait_time(self, scheduler, job, dispatcher, freeze_time):
...
class SchedulerImplementationTestBase(object):
    ...
    def dispatcher(self, scheduler):
        scheduler.add_dispatcher(DebugDispatcher())
