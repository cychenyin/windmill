...
If, for some reason, pip won't work, you can manually `download the hoi distribution
<https://pypi.python.org/pypi/hoi/>`_ from PyPI, extract and then install it::
...
hoi in different ways. The examples can also be
`browsed online <https://bitbucket.org/agronholm/hoi/src/master/examples/?at=master>`_.


Basic concepts
--------------

hoi has four kinds of components:
...
Your choice of scheduler depends mostly on your programming environment and what you'll be using hoi for.
Here's a quick guide for choosing a scheduler:

* :class:`~windmill.schedulers.blocking.BlockingScheduler`:
  use when the scheduler is the only thing running in your process
* :class:`~windmill.schedulers.background.BackgroundScheduler`:
  use when you're not using any of the frameworks below, and want the scheduler to run in the background inside your
  application
* :class:`~windmill.schedulers.asyncio.AsyncIOScheduler`:
  use if your application uses the asyncio module
* :class:`~windmill.schedulers.gevent.GeventScheduler`:
  use if your application uses gevent
* :class:`~windmill.schedulers.tornado.TornadoScheduler`:
  use if you're building a Tornado application
* :class:`~windmill.schedulers.twisted.TwistedScheduler`:
  use if you're building a Twisted application
* :class:`~windmill.schedulers.qt.QtScheduler`:
...
(:class:`~windmill.jobstores.memory.MemoryJobStore`). But if you need your jobs to persist over scheduler restarts or
...
:class:`~windmill.jobstores.sqlalchemy.SQLAlchemyJobStore` on a `PostgreSQL <http://www.postgresql.org/>`_ backend is
...
Otherwise, the default :class:`~windmill.executors.pool.ThreadPoolExecutor` should be good enough for most purposes.
If your workload involves CPU intensive operations, you should consider using
:class:`~windmill.executors.pool.ProcessPoolExecutor` instead to make use of multiple CPU cores.
...
hoi provides many different ways to configure the scheduler. You can use a configuration dictionary or you can
...
:class:`~windmill.schedulers.base.BaseScheduler` class. Scheduler subclasses may also have additional options which
...
Method 1::
    ...
    from windmill.schedulers.background import BackgroundScheduler
    from windmill.jobstores.mongodb import MongoDBJobStore
    from windmill.jobstores.sqlalchemy import SQLAlchemyJobStore
    from windmill.executors.pool import ThreadPoolExecutor, ProcessPoolExecutor
...
Method 2::
    ...
    # The "windmill." prefix is hard coded
    scheduler = BackgroundScheduler({
        'windmill.jobstores.mongo': {
             'type': 'mongodb'
        },
        'windmill.jobstores.default': {
            'type': 'sqlalchemy',
            'url': 'sqlite:///jobs.sqlite'
        },
        'windmill.executors.default': {
            'class': 'windmill.executors.pool:ThreadPoolExecutor',
            'max_workers': '20'
        },
        'windmill.executors.processpool': {
            'type': 'processpool',
            'max_workers': '5'
        },
        'windmill.job_defaults.coalesce': 'false',
        'windmill.job_defaults.max_instances': '3',
...
Starting the scheduler is done by simply calling :meth:`~windmill.schedulers.base.BaseScheduler.start` on the
scheduler. For schedulers other than `~windmill.schedulers.blocking.BlockingScheduler`, this call will return
immediately and you can continue the initialization process of your application, possibly adding jobs to the scheduler.

For BlockingScheduler, you will only want to call :meth:`~windmill.schedulers.base.BaseScheduler.start` after you're
...
#. by calling :meth:`~windmill.schedulers.base.BaseScheduler.add_job`
#. by decorating a function with :meth:`~windmill.schedulers.base.BaseScheduler.scheduled_job`

The first way is the most common way to do it. The second way is mostly a convenience to declare jobs that don't change
during the application's run time. The :meth:`~windmill.schedulers.base.BaseScheduler.add_job` method returns a
:class:`windmill.job.Job` instance that you can use to modify or remove the job later.
...
#. by calling :meth:`~windmill.schedulers.base.BaseScheduler.remove_job` with the job's ID and job store alias
#. by calling :meth:`~windmill.job.Job.remove` on the Job instance you got from
...
:class:`~windmill.job.Job` instance you received when adding the job. For jobs scheduled via the
:meth:`~windmill.schedulers.base.BaseScheduler.scheduled_job`, the first way is the only way.
...
You can easily pause and resume jobs through either the :class:`~windmill.job.Job` instance or the scheduler itself.
...
* :meth:`windmill.job.Job.pause`
* :meth:`windmill.schedulers.base.BaseScheduler.pause_job`

To resume:

* :meth:`windmill.job.Job.resume`
* :meth:`windmill.schedulers.base.BaseScheduler.resume_job`
...
:meth:`~windmill.schedulers.base.BaseScheduler.get_jobs` method. It will return a list of
:class:`~windmill.job.Job` instances. If you're only interested in the jobs contained in a particular job store,
then give a job store alias as the second argument.

As a convenience, you can use the :meth:`~windmill.schedulers.base.BaseScheduler.print_jobs` method which will print
...
You can modify any job attributes by calling either :meth:`windmill.job.Job.modify` or
:meth:`~windmill.schedulers.base.BaseScheduler.modify_job`. You can modify any Job attributes except for ``id``.
...
:meth:`windmill.job.Job.reschedule` or :meth:`~windmill.schedulers.base.BaseScheduler.reschedule_job`.
...
:meth:`~windmill.schedulers.base.BaseScheduler.add_listener`, OR'ing the different constants together.
The listener callable is called with one argument, the event object.

See the documentation for the :mod:`~windmill.events` module for specifics on the available events and their
attributes.
