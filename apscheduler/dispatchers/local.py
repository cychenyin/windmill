...
from apscheduler.dispatchers.base import BaseDispatcher,DispatchCode
from apscheduler.history import add_log, add_result
from kazoo.client import KazooClient, KazooState
from tzlocal import get_localzone
from datetime import datetime
from apscheduler.util import local_timestamp_to_datetime, datetime_to_utc_timestamp
...
# from apscheduler.events import JobExecutionEvent, EVENT_SCHEDULER_START, EVENT_SCHEDULER_SHUTDOWN,EVENT_JOBSTORE_ADDED,EVENT_JOBSTORE_REMOVED, EVENT_JOB_ADDED, EVENT_JOB_REMOVED, EVENT_JOB_MODIFIED, EVENT_JOB_EXECUTED, EVENT_JOB_ERROR, EVENT_JOB_MISSED

class ZkDispather(BaseDispatcher):
    '''
    ...
    def __init__(self, scheduler, executor, zk_hosts='127.0.0.1:2181', zk_root='/windmill'):
        super(ZkDispather, self).__init__(scheduler, executor)
    ...
    def dispatch(self, job, run_times):
        ...
        with self._lock:            
            if job.conf.exist_strategy == 1 and len(self._state[job.id]) > 0:
                logging.info('job ignored cause of exist_strategy=1 and the last running in local host still going on. id=%s name=%s run_time=%s host=%s' % (job.id, job.conf.name, run_time, self.ip ))
                run_time = None 
