...
import six, sys, logging,socket, json, time

from windmill.schedulers import SchedulerAlreadyRunningError, SchedulerNotRunningError, SchedulerInitError
from windmill.dispatchers.base import MaxInstancesReachedError,  DispatchCode
from windmill.executors.base import BaseExecutor
from windmill.executors.pool import ThreadPoolExecutor 
from windmill.stores.base import ConflictingIdError, JobLookupError, BaseJobStore
from windmill.stores.mysqlstore import MysqlJobStore
from windmill.jobruntime import JobRuntime
# from windmill.triggers.base import BaseTrigger
from windmill.util import asbool, asint, astimezone, maybe_ref, timedelta_seconds, undefined
from windmill.events import (
...
class BaseScheduler(six.with_metaclass(ABCMeta)):
    ...
    def configure(self, gconfig={}, prefix='', **options):
        """
...
#         config.update(options)
    ...
    def add_executor(self, executor, alias='default', **executor_opts):
        ...
        :param str|unicode|windmill.executors.base.BaseExecutor executor: either an executor instance or the name of
            an executor plugin
    ...
    def add_job(self, func, trigger=None, args=None, kwargs=None, id=None, name=None, misfire_grace_time=undefined,
        ...
        :param str|windmill.triggers.base.BaseTrigger trigger: trigger that determines when ``func`` is called
...
#         job_kwargs = dict((key, value) for key, value in six.iteritems(job_kwargs) if value is not undefined)
        job = JobRuntime(self, scheduler=self)
        job._setconf(conf)
    ...
    def _configure(self, conf):
        ...
        self._zk_hosts = self.config['ZK_URI']
        self._zk_root = self.config['ZK_ROOT']
    ...
    def _process_jobs(self):
        ...
        with self._jobstore_lock:
                for job in self._jobstore.get_due_jobs(now):
                    ...
                    job._calculate_run_times(now)
                    #run_times = job.current_run_times = 
                    ...
                    if run_times:
                        ...
                        if code == DispatchCode.DONE:
                            ...
                            if job_next_run:
                                ...
                                self._history.add_log(job.conf, output='job will NOT be run any more, so remove it.')
    ...
    def __zk_ensure(self):
        self._zk.ensure_path(self._zk_root + '/worker' )
        self._zk.ensure_path(self._zk_root + '/task' )
    
    def _self_register(self):
        self.__zk_ensure()
        host =socket.gethostname()
        
        data = json.dumps({"host":host, "start": datetime.now(self.timezone).strftime('%Y-%m-%d %H:%M:%S') })
        self._zk.create(self._zk_root + '/worker/' + host,value=data, ephemeral=True, makepath=True)
        
    def _release_job(self, job):
        try:
            path = self._zk_root + '/task/' + str(job.conf.id)
            #// TODO ... check exists issue 
            if not self._zk.exists(path):
                data = json.dumps({"hosts":job.conf.get_hosts_list(), "host_stretagy": job.conf.host_strategy })
                #print(data)
                self._zk.create(path,value=data, ephemeral=False, makepath=True)
                
                next = path + '/next'
                data = str(job.next_run_time)
                self._zk.create(next,value=data, ephemeral=False, makepath=True)
                
                current = path + '/current'
                data = ''
                self._zk.create(current,value=data, ephemeral=False, makepath=True)
            
                if job.conf.host_strategy == 0:
                    everyone = current + '/everyone'
                    data = ''
                    #self._zk.create(everyone, value=data, ephemeral=False, makepath=True)        
                    for h in job.get_hosts_list():
                        hostpath = everyone + '/' + h
                        self._zk.create(everyone,value=data, ephemeral=False, makepath=True)
        except KazooException, e:
            logging.info('exception when access to zk, msg=%s' % str(e) )
