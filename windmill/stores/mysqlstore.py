...
import logging
from windmill.stores.base import BaseJobStore, JobLookupError, ConflictingIdError
from windmill.util import maybe_ref, datetime_to_utc_timestamp, local_timestamp_to_datetime, datetime_to_timestamp, from_datetime_to_timestamp
...
try:
    from sqlalchemy import create_engine, Table, Column, MetaData, Unicode, String, Integer, Float, LargeBinary, select, text, and_
...
class MysqlJobStore(BaseJobStore):
    ...
    def __init__(self, url=None, engine=None, tablename='hoi_jobs', metadata=None,
                 pickle_protocol=pickle.HIGHEST_PROTOCOL, scheduler=None):
...
#         return self._get_jobs(self.jobs_t.c.next_run_time <= timestamp)
    def get_due_jobs(self, now):
        timestamp = datetime_to_utc_timestamp(now)
        return self._get_jobs( and_( self.wm_jobs_t.c.next_run_time <= timestamp, and_(self.wm_jobs_t.c.start_date <= timestamp, self.wm_jobs_t.c.end_date > timestamp )) )
...
#         return job
    def _reconstitute_job(self, row):
        ...
        job = JobRuntime.__new__(JobRuntime)
        job._scheduler = self._scheduler
        job._setconf(conf);
        #job._jobstore_alias = self._alias
...
#         t = windmill.util.convert_to_ware_datetime(t, get_localzone(), 'conf.next_run_time' )
#         state = {
#              'version': 1,
#              'conf': conf, 
#              'id': conf.id, 
...
#       selectable = selectable.order_by(self.wm_jobs_t.c.next_run_time)
        ...
        if failed_job_ids:
            # delete = self.jobs_t.delete().where(self.jobs_t.c.id.in_(failed_job_ids))
            # logic delete
            msg = 'job %s update status to 2 cause of failing to _reconstitute_job' % ','.join([ str(x) for x in list(failed_job_ids)])
            logging.error(msg)
            exit(1);
            ...
            conf.id = 0
            conf.cmd = ' '
            self._scheduler._history.add_log(conf, output=msg)
