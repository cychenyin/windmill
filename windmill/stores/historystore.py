...
from windmill.util import maybe_ref,datetime_to_utc_timestamp # local_timestamp_to_datetime, 
...
class HistoryStore(six.with_metaclass(ABCMeta)):
    ...
    def __init__(self, url=None, engine=None, metadata=None, scheduler=None):
    ...
    def __now(self):
        return int(datetime_to_utc_timestamp(datetime.now(self._scheduler.timezone)))
...
#             logging.error("fail to update history cause of id %s not found" % id)
    ...
    def add_log(self, jobconf, run_time=0, output=None, alarm_phones='', alarm_mails=''):
        run_time = int(run_time)
        msg = self.createMessage(jobconf, run_time) 
        msg['info_type'] = 0
        if output:
            msg['output'] = output
        if alarm_phones:
    ...
    def add_result(self, jobconf, run_time=0, result=1, output=None, cost_ms=-1, alarm_phones='', alarm_mails=''):
        run_time = int(run_time)
        msg = self.createMessage(jobconf, run_time) 
        msg['info_type'] = 1
        if output:
